...
def fetch_history(scheme_code):
    ...
    date_object = [datetime.strptime(date, '%d-%m-%Y').date() for date in dates]
...
def screen_feat(Nav_hist):
    ...
    for iter in time_delta:
        ...
        per_value[i] = np.sum(tt.pct_change().values[1:])#*np.float(weig[i])#/np.float(iter[0])
...
schemes_df = pd.DataFrame({ 'Name': list(schemes.values()), 'Code': list(schemes.keys()) }) 
...
for scheme_idx in tqdm(range(len(codes)-1)):
    code = codes[scheme_idx]
